...
// 25% larger at lg+ viewports (40px → 50px)
const ICON_SPAN = 'w-10 h-10 lg:w-[50px] lg:h-[50px]';
...
const logos: LogoEntry[] = [
  ...
  {
    // Pipedream wordmark — viewBox 512×100, rendered as a wide short mark
    id: 'pipedream',
    label: 'Pipedream',
    svgPath: pipedreamPath,
    viewBox: '0 0 512 100',
    spanClass: 'w-[88px] h-[17px] lg:w-[110px] lg:h-[21px]',
...
export default function FloatingLogos() {
  return (
    <div aria-hidden="true" className="absolute inset-0 pointer-events-none overflow-hidden">
      {logos.map((logo) => (
        // Outer div: continuous y float only
        <motion.div
          key={logo.id}
        ...
        >
          {/* Inner div: color/scale/filter tracked as motion values for hover */}
          <motion.div
            title={logo.label}
            initial={{
          ...
          >
            {logo.svgPath ? (
              <span className={`block ${logo.spanClass ?? ICON_SPAN}`}>
                <svg
                  role="img"
              ...
              // Monogram fallback
              <div
                className="w-10 h-10 lg:w-[50px] lg:h-[50px] rounded-md flex items-center justify-center text-xs font-bold font-mono"
                style={{
